...
class DictionaryManager {
    constructor(dictionaryPath, fullDictionaryPath) {
        this._dictionaryPath = dictionaryPath;
        this._fullDictionaryPath = fullDictionaryPath;

        this._excludedServiceNames = ["application-managing", "speech-synthesis", "arduino-moving"];
                ...
                return this.exec(`cat ${this._fullDictionaryPath} | grep -e '^${word}[ |(]'`)
                    .catch((err) => {
                        if (err instanceof bluebird.OperationalError) {
                            throw new errors.ValidationError(`There is no '${word}' word in the main dictionary`);
                        }
                        throw err;
                    });
            })
